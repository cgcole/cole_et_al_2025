...
gene_callers_id <- read_tsv("../metadata/gene_callers_id.txt") %>% 
  filter(subset != 7) %>% 
  full_join(lan_contig_gene_calls_7 %>% 
...
lan_contig_gene_calls_7 <- read_tsv("../metadata/lan_contig_gene_calls.txt", num_threads = 5) %>%
  dplyr::rename(original_gene_callers_id = gene_callers_id) %>%
...
lan_contig_id_subset_07 <- lan_contig_gene_calls_7 %>% 
  distinct(contig)

write_tsv(lan_contig_id_subset_07, "../metadata/lan_contig_id_subset_07.txt", col_names = FALSE)

rm(lan_contig_id_subset_07)

# Save fasta file of all lan contigs genes for GhostKOALA annotation
write.fasta(as.list(lan_contig_gene_calls_7$aa_sequence), lan_contig_gene_calls_7$contig_gene_id, "../fasta_files/lan_contig_genes_7.fa")
write.fasta(as.list(lan_contig_gene_calls_7$aa_sequence[1:1632]), lan_contig_gene_calls_7$contig_gene_id[1:1632], "../fasta_files/lan_contig_genes_subset_17.fa")

# Save gene calls to input into anvi-gen-contig-database to make contig database for annotation in Anvio
lan_contig_annotation_gene_calls_7 <- lan_contig_gene_calls_7 %>% 
  select(-contig_gene_id, -original_gene_callers_id, -subset)%>% 
  relocate(gene_callers_id)

write_tsv(lan_contig_annotation_gene_calls_7, "../lan_contig_annotations/anvio_annotations/lan_contig_annotation_gene_calls_7.txt")

rm(lan_contig_annotation_gene_calls_7)
