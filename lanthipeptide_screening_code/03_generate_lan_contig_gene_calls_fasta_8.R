...
lan_contig_gene_calls_8 <- read_tsv("../metadata/lan_contig_gene_calls.txt", num_threads = 5) %>% 
  dplyr::rename(original_gene_callers_id = gene_callers_id) %>% 
  full_join(read_tsv("../metadata/gene_callers_id.txt", num_threads = 5) %>% 
              filter(subset %in% c(1,2,3,4,5,6,7))) %>% 
...
gene_callers_id <- read_tsv("../metadata/gene_callers_id.txt") %>% 
  filter(subset != 8) %>% 
  full_join(lan_contig_gene_calls_8 %>% 
...
lan_contig_gene_calls_8 <- read_tsv("../metadata/lan_contig_gene_calls.txt") %>%
  dplyr::rename(original_gene_callers_id = gene_callers_id) %>%
  right_join(read_tsv("../metadata/gene_callers_id.txt")) %>%
  arrange(gene_callers_id) %>%
  filter(subset == 8) %>%
  mutate(contig_gene_id = paste0(contig, "_gene_", gene_callers_id))
...
lan_contig_id_subset_08 <- lan_contig_gene_calls_8 %>% 
  distinct(contig)

write_tsv(lan_contig_id_subset_08, "../metadata/lan_contig_id_subset_08.txt", col_names = FALSE)

rm(lan_contig_id_subset_08)

# Save fasta file of all lan contigs genes for GhostKOALA annotation
write.fasta(as.list(lan_contig_gene_calls_8$aa_sequence), lan_contig_gene_calls_8$contig_gene_id, "../fasta_files/lan_contig_genes_8.fa")
write.fasta(as.list(lan_contig_gene_calls_8$aa_sequence[1:198743]), lan_contig_gene_calls_8$contig_gene_id[1:198743], "../fasta_files/lan_contig_genes_subset_18.fa")


# Save gene calls to input into anvi-gen-contig-database to make contig database for annotation in Anvio
lan_contig_annotation_gene_calls_8 <- lan_contig_gene_calls_8 %>% 
  select(-contig_gene_id, -original_gene_callers_id, -subset)%>% 
  relocate(gene_callers_id)

write_tsv(lan_contig_annotation_gene_calls_8, "../lan_contig_annotations/anvio_annotations/lan_contig_annotation_gene_calls_8.txt")

rm(lan_contig_annotation_gene_calls_8)
