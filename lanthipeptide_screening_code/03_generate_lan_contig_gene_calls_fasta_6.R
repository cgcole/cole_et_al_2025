...
gene_callers_id <- read_tsv("../metadata/gene_callers_id.txt") %>% 
  filter(subset != 6) %>% 
  full_join(lan_contig_gene_calls_6 %>% 
...
lan_contig_gene_calls_6 <- read_tsv("../metadata/lan_contig_gene_calls.txt", num_threads = 5) %>%
  dplyr::rename(original_gene_callers_id = gene_callers_id) %>%
...
lan_contig_id_subset_06 <- lan_contig_gene_calls_6 %>% 
  distinct(contig)

write_tsv(lan_contig_id_subset_06, "../metadata/lan_contig_id_subset_06.txt", col_names = FALSE)

rm(lan_contig_id_subset_06)

# Save fasta file of all lan contigs genes for GhostKOALA annotation
write.fasta(as.list(lan_contig_gene_calls_6$aa_sequence), lan_contig_gene_calls_6$contig_gene_id, "../fasta_files/lan_contig_genes_6.fa")
write.fasta(as.list(lan_contig_gene_calls_6$aa_sequence[1:430958]), lan_contig_gene_calls_6$contig_gene_id[1:430958], "../fasta_files/lan_contig_genes_subset_16.fa")

# Save gene calls to input into anvi-gen-contig-database to make contig database for annotation in Anvio
lan_contig_annotation_gene_calls_6 <- lan_contig_gene_calls_6 %>% 
  select(-contig_gene_id, -original_gene_callers_id, -subset)%>% 
  relocate(gene_callers_id)

write_tsv(lan_contig_annotation_gene_calls_6, "../lan_contig_annotations/anvio_annotations/lan_contig_annotation_gene_calls_6.txt")

rm(lan_contig_annotation_gene_calls_6)
