...
lan_contig_gene_calls_4 <- read_tsv("../metadata/lan_contig_gene_calls.txt") %>% 
...
gene_callers_id <- read_tsv("../metadata/gene_callers_id.txt") %>% 
  filter(subset != 4) %>% 
  full_join(lan_contig_gene_calls_4 %>% 
...
lan_contig_gene_calls_4 <- read_tsv("../metadata/lan_contig_gene_calls.txt", num_threads = 5) %>%
  dplyr::rename(original_gene_callers_id = gene_callers_id) %>%
...
lan_contig_id_subset_04 <- lan_contig_gene_calls_4 %>% 
  distinct(contig)

write_tsv(lan_contig_id_subset_04, "../metadata/lan_contig_id_subset_04.txt", col_names = FALSE)

rm(lan_contig_id_subset_04)

# Save fasta file of all lan contigs genes for GhostKOALA annotation
write.fasta(as.list(lan_contig_gene_calls_4$aa_sequence), lan_contig_gene_calls_4$contig_gene_id, "../fasta_files/lan_contig_genes_4.fa")
write.fasta(as.list(lan_contig_gene_calls_4$aa_sequence[1:658]), lan_contig_gene_calls_4$contig_gene_id[1:658], "../fasta_files/lan_contig_genes_subset_13.fa")

# Save gene calls to input into anvi-gen-contig-database to make contig database for annotation in Anvio
lan_contig_annotation_gene_calls_4 <- lan_contig_gene_calls_4 %>% 
  select(-contig_gene_id, -original_gene_callers_id, -subset)%>% 
  relocate(gene_callers_id)

write_tsv(lan_contig_annotation_gene_calls_4, "../lan_contig_annotations/anvio_annotations/lan_contig_annotation_gene_calls_4.txt")

rm(lan_contig_annotation_gene_calls_4)
